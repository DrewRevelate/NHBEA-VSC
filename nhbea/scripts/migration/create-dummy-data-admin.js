// Script to create dummy data for all FireCMS collections using Admin SDK
// Run with: node scripts/create-dummy-data-admin.js
...
async function createDummyData() {
  try {
    ...
    const conferenceRef = await db.collection('conference').add({
    ...
    await db.collection('registrants').add({
    ...
    await db.collection('pastPresidents').add({
    ...
    await db.collection('studentApplicants').add({
      personalInfo: {
        firstName: "Emma",
        lastName: "Wilson",
    ...
    await db.collection('awardNominations').add({
      nomineeInfo: {
